    ...
    <div class="card">
        ...
        <div class="payment-top-tab mt-3 mb-3">
            ...
            <ul class="nav nav-tabs card-header-tabs align-items-end">
                ...
                <li class="nav-item">
                    <a class="nav-link arroPayMaya_active_label" href="{!! url('settings/payment/arropay-maya') !!}"><i
                            class="fa fa-envelope-o mr-2"></i>{{trans('lang.arroPayMaya')}}<span
                            class="badge ml-2"></span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link arroPayMayaQr_active_label" href="{!! url('settings/payment/arropay-maya-qr') !!}"><i
                            class="fa fa-envelope-o mr-2"></i>{{trans('lang.arroPayMayaQr')}}<span
                            class="badge ml-2"></span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link arroPayInstapay_active_label" href="{!! url('settings/payment/arropay-instapay') !!}"><i
                            class="fa fa-envelope-o mr-2"></i>{{trans('lang.arroPayInstapay')}}<span
                            class="badge ml-2"></span>
                    </a>
                </li>
                <li class="nav-item">
                    <a class="nav-link arroPayAuth_active_label" href="{!! url('settings/payment/arropay-auth') !!}"><i
                            class="fa fa-envelope-o mr-2"></i>{{trans('lang.arroPayAuth')}}<span
                            class="badge ml-2"></span>
                    </a>
                </li>
...
var arroPayMayaSettings = database.collection('settings').doc('arropay_maya_settings');
var arroPayMayaQrSettings = database.collection('settings').doc('arropay_maya_qr_settings');
var arroPayInstapaySettings = database.collection('settings').doc('arropay_instapay_settings');
var arroPayAuthSettings = database.collection('settings').doc('arropay_auth_settings');
...
$(document).ready(function() {
    ...
    ref.get().then(async function(snapshots) {
        ...
        arroPayMayaSettings.get().then(async function(arroPayMayaSettings) {
            var arropay = arroPayMayaSettings.data();
            if (arropay && arropay.enable) {
                jQuery(".arroPayMaya_active_label span").addClass('badge-success');
                jQuery(".arroPayMaya_active_label span").text('Active');
            }
        })

        arroPayMayaQrSettings.get().then(async function(arroPayMayaQrSettings) {
            var mayaQr = arroPayMayaQrSettings.data();
            if (mayaQr && mayaQr.enable) {
                jQuery(".arroPayMayaQr_active_label span").addClass('badge-success');
                jQuery(".arroPayMayaQr_active_label span").text('Active');
            }
        })

        arroPayInstapaySettings.get().then(async function(arroPayInstapaySettings) {
            var instapay = arroPayInstapaySettings.data();
            if (instapay && instapay.enable) {
                jQuery(".arroPayInstapay_active_label span").addClass('badge-success');
                jQuery(".arroPayInstapay_active_label span").text('Active');
            }
        })

        arroPayAuthSettings.get().then(async function(arroPayAuthSettings) {
            var auth = arroPayAuthSettings.data();
            if (auth && auth.token) {
                jQuery(".arroPayAuth_active_label span").addClass('badge-success');
                jQuery(".arroPayAuth_active_label span").text('Active');
            }
        })
