    ...
    <script>
        ...
        async function getTotalEarnings(filterType = null, year = null, month = null, startDate = null, endDate = null) {
            ...
            if (filterType === 'year' && year) {
                ...
                lastStartTS = kweekDb.Timestamp.fromDate(new Date(year - 1, 0, 1));
                lastEndTS = kweekDb.Timestamp.fromDate(new Date(year - 1, 11, 31, 23, 59, 59));
                ...
                lastStartTS = kweekDb.Timestamp.fromDate(new Date(prevYear, prevMonth - 1, 1));
                lastEndTS = kweekDb.Timestamp.fromDate(new Date(prevYear, prevMonth, 0, 23, 59, 59));
                ...
                lastStartTS = kweekDb.Timestamp.fromDate(lastStart);
                lastEndTS = kweekDb.Timestamp.fromDate(lastEnd);
            }
        ...
        async function loadOrderStatusCounts(filterType = null, year = null, month = null, startDate = null, endDate = null, active_id) {
            ...
            if (filterType === 'year' && year) {
                ...
                startTS = kweekDb.Timestamp.fromDate(startOfYear);
                endTS = kweekDb.Timestamp.fromDate(endOfYear);
            } else if (filterType === 'month' && year && month) {
                let startOfMonth = new Date(year, month - 1, 1);
                let endOfMonth = new Date(year, month, 0, 23, 59, 59);
                startTS = kweekDb.Timestamp.fromDate(startOfMonth);
                endTS = kweekDb.Timestamp.fromDate(endOfMonth);
            } else if (filterType === 'custom' && startDate && endDate) {
                let start = toStartOfDay(startDate);
                let end = toEndOfDay(endDate);
                startTS = kweekDb.Timestamp.fromDate(start);
                endTS = kweekDb.Timestamp.fromDate(end);
            }
        ...
        function buildRidesHTML(snapshots) {
            ...
            snapshots.docs.forEach((listval) => {
                ...
                const rideBilling = calculateRideEarnings(val, /^\d+$/, /^((\d+(\.\d *)?)|((\d*\.)?\d+))$/);
                html = html + '<td data-url="' + route + '" class="redirecttopage">' + formatCurrencyAmount(rideBilling.commission) + '</td>';
        ...
        function loadVendorDashboardData(filterType = null, year = null, month = null, startDate = null, endDate = null, active_id, active_type) {
            ...
            // 🔹 database timestamps
            const startThisTS = kweekDb.Timestamp.fromDate(startOfThisPeriod);
            const endThisTS = kweekDb.Timestamp.fromDate(endOfThisPeriod);
            const startLastTS = startOfLastPeriod ? kweekDb.Timestamp.fromDate(startOfLastPeriod) : null;
            const endLastTS = endOfLastPeriod ? kweekDb.Timestamp.fromDate(endOfLastPeriod) : null;
