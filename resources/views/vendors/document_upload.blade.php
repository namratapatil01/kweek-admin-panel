...
<div class="page-wrapper">
    ...
    trans('lang.save')}}
                    ...
                    <a href="{{url('vendors/document-list/' . $ownerId)}}" class="btn btn-default"><i
                            class="fa fa-undo"></i>{{
    trans('lang.cancel')}}</a>
                </div>
            </div>
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            var loadUrl = "{{ route('vendors.get-document-upload', [':ownerId', ':id']) }}"
                            .replace(':ownerId', id)
        ...
        $(document).on('click', '.save-form-btn', function () {
            ...
            if (backSide && back_photo == "") {
                ...
                storeImageData().then(IMG => {
                    $.ajax({
                        url: "{{ route('vendors.save-document-upload') }}",
                        type: "POST",
                        data: {
                            _token:     "{{ csrf_token() }}",
                            id:         id,
