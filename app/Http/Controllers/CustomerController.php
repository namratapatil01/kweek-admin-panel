...
class CustomerController extends Controller
{
    ...
    protected function buildDatatableRow($record): array
    {
        ...
        // Checkbox
        $row[] = $canDelete ? '<div class="d-flex align-items-center"><input type="checkbox" class="row-select is_active" id="row_' . e($id) . '" data-id="' . e($id) . '"><label class="control-label mb-0" for="row_' . e($id) . '" style="min-width: 20px; padding-left: 0;"></label></div>' : '';
        ...
        $userInfo = '<div class="d-flex align-items-center">
                        ...
                        <div>
                            <span class="font-weight-bold" style="color: #000; text-decoration: underline;">' . $name . '</span>
                        </div>
                    </div>';
        $row[] = $userInfo;
        ...
        // Contact Info
        $contactInfo = '<div>
                            <div>' . e($record->email ?: '-') . '</div>
                            <div>' . e($record->phoneNumber ?: '-') . '</div>
                        </div>';
        $row[] = $contactInfo;

        // Date
        $dateStr = $record->created_at ? $record->created_at->format('D M d Y h:i:s A') : '-';
        $row[] = '<div style="font-size: 0.9rem;">' . $dateStr . '</div>';

        // Active Toggle
        $isActive = filter_var($record->active, FILTER_VALIDATE_BOOLEAN);
        $checked = $isActive ? 'checked' : '';
        $row[] = '<label class="switch"><input type="checkbox" class="is_active" id="' . e($id) . '" ' . $checked . '><span class="slider round"></span></label>';
        ...
        $actions .= '<a href="' . route('users.walletstransaction', $id) . '" title="Wallet History" style="' . $btnStyle . ' color: #d97706; border-color: #fcd34d; background: #fff;"><i class="ri-wallet-line" style="font-size: 16px;"></i></a>';
