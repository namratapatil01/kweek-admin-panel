        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="{!! route('deliveryman') !!}">{{trans('lang.deliveryman')}}</a></li>
...
<script>
            ...
            if (docSnapshot.docs.length) {
                ...
                documents.forEach((ele) => {
                    ...
                    docRefs.get().then(async function (docrefSnapshot) {
                        ...
                        trhtml += '<a href="' + (`/deliveryman/document/upload/` + id.trim() + `/` + doc.id.trim()) + '" data-id="' + doc.id + '"><i class="mdi mdi-lead-pencil" title="Edit"></i></a>&nbsp;';
    ...
    $(document).on('click', '.verify-doc', function () {
        ...
        docRefsTmp.get().then(async function (docrefSnapshot) {
            var docData = docrefSnapshot.exists ? docrefSnapshot.data() : null;
            var objects = (docData && docData.documents) ? docData.documents : [];
            var keydataId = objects.findIndex((doc) => doc.documentId == docId);

            if (keydataId >= 0) {
                objects[keydataId].status = status;
            } else {
                objects.push({ documentId: docId, status: status, frontImage: '', backImage: '' });
            }

            var writePromise = docrefSnapshot.exists
                ? database.collection('documents_verify').doc(id).update({ documents: objects })
                : database.collection('documents_verify').doc(id).set({ documents: objects });

            writePromise.then(async function () {
